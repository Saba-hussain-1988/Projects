...
function StudentInfo(name, DOB, balance, IDnumber, dayAndTime, course, isFeePad) {
    return {
        fullName: name,
        DOB: DOB,
...
function convertToUpperCase(value) {
    let converted = value.toUpperCase();
    return converted;
}
;
//generate five digits ID code
function generateIDcode(value) {
    let ID = ``;
    if (value < 10) {
        ID = `0000${value}`;
    }
